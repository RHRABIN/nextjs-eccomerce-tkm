...
const MobileNav = ({ user, toggle, setToggle, handleLogout, loginSuccess }) => {
    ...
    return (
        <nav className={`bg-secondary overflow-hidden text-[#888888] px-4 transition-all ease-in-out duration-700 ${toggle ? 'max-h-[2000px] overflow-y-hidden lg:overflow-hidden lg:invisible lg:h-0 visible  pb-4' : 'max-h-0 overflow-hidden invisible'}`}>
            <ul className='pt-2'>
                {categories?.length > 0 &&
                    categories?.map(category => category?.children?.length > 0 && toggle ?
